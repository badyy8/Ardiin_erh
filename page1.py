...
with tab3:
    ...
    with st.expander(expanded=True,label= 'Тайлбар:'):
        ...
        st.markdown(f"""
        - Сард дунджаар **{reached_1000_df['1000 оноо хүрсэн'].mean():,.0f}** хэрэглэгч 1000 онооны босгыг давсан нь,
...
with tab2:
    ...
    with st.expander("Тайлбар", expanded=True):
        ...
        st.markdown(
            """
            - **0–199 оноо:** **77.4% – 84.1%** – давамгай сегмент, бага идэвхитэй  
            - **200–499 оноо:** **7.7% – 10.1%** – тогтвортой дунд сегмент  
            - **500–999 оноо:** **1.6% – 4.3%** – өсөх боломжтой сегмент  
            - **1000+ оноо:** **4.3% – 10.1%** – өндөр үнэ цэнэтэй
            """
        )
...
with tab1:
    ...
    with st.expander(expanded=True, label='Тайлбар:'):
        ...
        st.markdown(f"""
        - 2025 оны **1–12** дугаар саруудад нийт **{df['CUST_CODE'].nunique():,}** хэрэглэгч урамшууллын хөтөлбөрт хамрагдсан байна.
